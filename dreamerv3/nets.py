...
class VPR(nj.Module):
  ...
  def obs_step(self, prev_state, prev_action, embed, is_first):
    ...
    prev_state = jax.tree_util.tree_map(
        ...
        prev_state, self.initial(len(is_first)))
    prior = self.img_step(prev_state, prev_action)
    x = jnp.concatenate([prior['deter'], jnp.reshape(embed, (-1, prior['deter'].shape[1]))], -1)
    x = self.get('obs_out', Linear, **self._kw)(x)
    stats = self._stats('obs_stats', x)
    dist = self.get_dist(stats)
    stoch = dist.sample(seed=nj.rng())
...
class GRU(nj.Module):
  ...
  def __call__(self, inputs, state):
    ...
    if state is None:
      state = self.initial(action.shape[0])
    step = lambda prev, inputs: self.observe(prev[0], *inputs)
    inputs = swap(action), swap(embed), swap(is_first)
    start = state, state
    x = jaxutils.scan(step, inputs, start, self._unroll)
    #post = {k: swap(v) for k, v in post.items()}
    #prior = {k: swap(v) for k, v in prior.items()}
